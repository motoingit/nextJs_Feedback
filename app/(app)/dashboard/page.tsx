...
import DefaultMessageCard from "@/components/my/DefaultMessageCard";
...
export default function Dashboard() {
  ...
  const handleSwitchChange = async () => {
    ...
    try {
      ...
      toast.error("Error", {
        description:
          axiosError.response?.data.message ??
          "Failed to toggle message acceptance status",
        duration: 1000,
      });
    } finally {
      setIsSwitchLoading(false);
  ...
  const username =
    session?.user?.username || (session?.user as User)?.email || "";
      ...
      <div className="mx-auto max-w-6xl space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-2 border border-border/50 bg-card/70 backdrop-blur-sm rounded-2xl p-6 shadow-sm flex flex-col justify-between">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Share this link on your social profiles to receive anonymous
                messages.
              </p>
            </div>
          ...
          <Card className="border border-border/50 bg-card/70 backdrop-blur-sm rounded-2xl p-6 shadow-sm flex flex-col justify-between">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mt-6 bg-slate-100/50 dark:bg-slate-900/30 border border-border/30 rounded-xl p-3">
